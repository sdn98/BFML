...
class PytorchFLModel(FLModel):
    ...
    def evaluate_model(self, x, y, eval_metrics=None, **kwargs):
        ...
        accuracy = str(100 * metrics.accuracy_score(y, y_pred_argmax))
        length = len(metrics.precision_score(y, y_pred_argmax, average=None))
        precision = str(100 * sum(metrics.precision_score(y, y_pred_argmax, average=None))/length)
        recall = str(100 * sum(metrics.recall_score(y, y_pred_argmax, average=None))/length)
        fone= str(100 * sum(metrics.f1_score(y, y_pred_argmax, average=None))/length)
        loss = str(self.model.get_loss(torch.from_numpy(y_pred), torch.from_numpy(y)).numpy())
        data = '{ library: IBM; \n accuracy: ' + accuracy + '; \n loss: ' + loss + '; \n recall: ' + recall + '; \n precision: ' + precision + '; \n f1: ' + fone + '; \n memory: ' + memory + '; \n }'
        print(data)
